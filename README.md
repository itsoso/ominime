...
- **Kim / 微信豆包输入法兼容**：旧版 Kim 会在发送前于本机识别输入区中可见的中文，微信继续读取已确认的豆包候选词；临时画面不会保存或上传，内容无法确认时只计数且不保存原始拼音。
...
默认会保存提交时的文字上下文元数据，例如应用、窗口标题、焦点控件和容器信息，用于 Web 复盘和捕获诊断；上下文结构不会重复保存辅助功能节点中的文字值，当前流程也不会为每次提交保存截图。
